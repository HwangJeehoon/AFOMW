% run_EMG_plots.m
% EMG_processed 결과에서 trial(bare/P1/P2/P3)별로 지정한 step 범위를 골라
% 근육별 RMS bar plot(요청 1)과 gait cycle mean profile plot(요청 2)을 그리고
% fig/EMG/{Subject}_{날짜}/에 png로 저장한다. (같은 subject/날짜 내에서만 비교)

clear; clc; close all;
addpath(fullfile(fileparts(mfilename('fullpath')), '..', 'src'));

rootDir = fullfile(fileparts(mfilename('fullpath')), '..');

%% ── 공통 설정 ──────────────────────────────────────────────────────
muscleNames = {'L_TA', 'L_GM', 'L_RF', 'L_VL', 'R_TA', 'R_GM', 'R_RF', 'R_VL'};

sideNames.L = 'Left';
sideNames.R = 'Right';
muscleFullNames.TA = 'TibAnt';
muscleFullNames.GM = 'Gastroc';
muscleFullNames.RF = 'RectFem';
muscleFullNames.VL = 'VastLat';

lpfCutoffHz = 10;  % Profile plot에 적용할 low-pass filter cutoff (Hz)

%% ── subject/날짜별 설정 (trial별 step 번호 범위를 직접 지정) ──────────
configs = struct('subject', {}, 'dateStr', {}, 'stepRange', {});

configs(1).subject = 'SAH01';
configs(1).dateStr = '260706';
configs(1).stepRange.bare = 3:73; %76
configs(1).stepRange.P1   = 18:88; %91
configs(1).stepRange.P2   = 7:77; %80
configs(1).stepRange.P3   = 5:75; %78

configs(2).subject = 'SAH01';
configs(2).dateStr = '260713';
configs(2).stepRange.bare = 3:73; %76
configs(2).stepRange.P1   = 4:74; %77
configs(2).stepRange.P2   = 8:78; %81
configs(2).stepRange.P3   = 3:73; %76

configs(3).subject = 'SAH03';
configs(3).dateStr = '260623';
configs(3).stepRange.bare = 4:74; %77
configs(3).stepRange.P1   = 51:121; %124
configs(3).stepRange.P2   = 3:73; %76
configs(3).stepRange.P3   = 23:93; %96

configs(4).subject = 'SAH03';
configs(4).dateStr = '260630';
configs(4).stepRange.bare = 4:74; %77
configs(4).stepRange.P1   = 14:84; %87
configs(4).stepRange.P2   = 15:85; %88
configs(4).stepRange.P3   = 15:85; %88

% configs(1).subject = 'SAH03';
% configs(1).dateStr = '260630';
% configs(1).stepRange.bare = 4:74; %77
% configs(1).stepRange.P1   = 14:84; %87
% configs(1).stepRange.P2   = 15:85; %88
% configs(1).stepRange.P3   = 15:85; %88

%% ── 데이터 로드 + plot ────────────────────────────────────────────
trialKeys = {'bare', 'P1', 'P2', 'P3'};

for i = 1:numel(configs)
    cfg = configs(i);

    trials = struct('name', {}, 'tables', {});
    for k = 1:numel(trialKeys)
        key = trialKeys{k};
        trials(k).name = key;
        trials(k).tables = loadTrialSteps(cfg.subject, cfg.dateStr, key, cfg.stepRange.(key), rootDir);
    end

    outDir = fullfile(rootDir, 'fig', 'EMG', sprintf('%s_%s', cfg.subject, cfg.dateStr));
    if ~exist(outDir, 'dir')
        mkdir(outDir);
    end

    plotRMSBarByMuscle(cfg.subject, cfg.dateStr, trials, muscleNames, sideNames, muscleFullNames, outDir);
    plotGaitCycleProfile(cfg.subject, cfg.dateStr, trials, muscleNames, sideNames, muscleFullNames, outDir, lpfCutoffHz);

    fprintf('\n=== plotting complete: %s ===\n', outDir);
end
